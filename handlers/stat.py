        ...
        try:
            if int(i[1]) == chat_id:
                await bot.send_message(chat_id=message.chat.id, text= f'<b>Канал:</b> {i[0]}\n\n'
                                                                      f'<b>Реферальная ссылка: https://t.me/InfobizSprintBot?start={i[0][1:]}</b>\n\n'
                                                                          f'<b>Неоплаченный трафик:</b> {i[3]}\n'
                                                                          f'<b>Трафика всего:</b> {i[2]}\n'
                                                                          f'<b>Реквезиты партнера:</b> {i[4]}',parse_mode='html',disable_web_page_preview=True)
        except:
            pass
...
async def bat_reg_support123321(message: types.Message):
    await message.answer(text="Напишите 'имя пользователя' своего телеграм (если его нет, установите и после напишите через @nalatinice)")
